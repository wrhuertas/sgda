...
export class BuscarUsuarioComponent implements OnInit {
  ...
  buscarUsuarios() {
    if (!this.filtro_usuario || this.filtro_usuario.trim() === '') {
      Swal.fire({
        icon: 'warning',
        title: 'Campo vacío',
        text: 'Por favor, ingrese la información o dato de búsqueda del usuario (nombre, cédula, etc.).',
        confirmButtonText: 'Entendido',
        confirmButtonColor: '#f6a032'
      });
      return;
    }
  
    if (this.filtro_usuario.trim().length < 3) {
      Swal.fire({
        icon: 'info',
  ...
  // Log cuando se asigna rol PARA para depuración
  onRolChange(p: any) {
    try {
      console.log('[AsignarTramite/BuscarUsuario] Rol cambiado:', p?.nombre_completo || p?.id, '->', p?.rol_envio);
      if (p?.rol_envio === 'PARA') {
        console.log('[AsignarTramite/BuscarUsuario] Seleccionado como PARA:', p);
      }
    } catch {}
  }
}
