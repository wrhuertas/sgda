...
export class VistaPreviaComponent implements OnInit {
  ...
  cargarEmpresa(idEmpresa: number) {
    this.AsignartramiteService.cargarempresaid(idEmpresa).subscribe({
      next: (empresa: any) => {
      ...
      content: [
        {
          columns: [
            { width: '*', text: '' },
            {
              width: 'auto',
              stack: [
                this.logoBase64 ? { image: this.logoBase64, width: 80, margin: [0, 0, 0, 6], alignment: 'right' } : { text: '' },
                nombreEmpresa ? { text: nombreEmpresa, style: 'empresa', margin: [0, 2, 0, 4], alignment: 'center' } : { text: '' },
                { text: `${tipoDocumento} Nro. ${numeroDocumento}`, style: 'docNumber', margin: [0, 2, 0, 2], alignment: 'right' },
                { text: `Quito, ${fecha}`, style: 'docDate', alignment: 'right' }
              ]
            }
          ]
        },
        ...
        // Listado de anexos (si existen)
        ...(Array.isArray(this.data?.anexos_nombres) && this.data.anexos_nombres.length > 0
          ? [
              { text: '\nAnexos:', style: 'label', margin: [0, 16, 0, 6] },
              { ul: this.data.anexos_nombres.map((n: any) => String(n)), fontSize: 9 }
            ]
          : [])
      ],
      styles: {
  ...
  formatLista(usuarios: any[]) {
    ...
    return usuarios
      .map(u => {
        const nombre = String(u?.nombre_completo || u?.full_name || u?.nombre || u?.name || '').trim();
        const titulo = String(u?.titulo || u?.titulo_usuario || '').trim();
        if (!nombre) return '';
        return titulo ? `${nombre} — ${titulo}` : nombre; // incluir título si existe
      })
      .filter(v => !!v)
      .join(', ') || 'No asignado';
  }
