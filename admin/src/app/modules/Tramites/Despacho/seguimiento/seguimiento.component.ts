...
export class SeguimientoComponent implements OnInit, OnDestroy {
  ...
  cargarSeguimiento(id_tramite: number) {
    this.DespachoService.seguimientosTramite(id_tramite).subscribe({
      next: (resp: any) => {
  ...
  private esActa(doc: any): boolean {
   ...
   cargarMovimientos(id_tramite: number) {
     this.DespachoService.movimientosTramite(id_tramite).subscribe({
       next: (resp: any) => {
  ...
  private async cargarPreviewPdf(doc: any): Promise<void> {
    ...
    try {
      const blob = await firstValueFrom(this.DespachoService.obtenerDocumentoTramite(doc.id_documento_tramite));
      if (!blob || blob.size === 0) {
        this.previewNoDisponible = true;
        this.toast.error('El documento no está disponible.');
        return;
  ...
  async abrirEnNuevaPestana() {
    ...
    try {
      const blob = await firstValueFrom(this.DespachoService.obtenerDocumentoTramite(this.selectedDoc.id_documento_tramite));
      if (!blob || blob.size === 0) {
        this.toast.error('El documento no está disponible.');
        return;
      }
