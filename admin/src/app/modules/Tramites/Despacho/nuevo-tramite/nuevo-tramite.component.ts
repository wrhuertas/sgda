...
export class NuevoTramiteComponent implements OnInit {
  ...
  private cargarTipos() {
    this.DespachoService.configTipoDocumento(this.id_empresa).subscribe({
      next: (resp: any) => {
    ...
    this.DespachoService.configTipoTramite(this.id_empresa).subscribe({
      next: (resp: any) => {
  ...
  private actualizarNumeroInterno() {
    ...
    this.DespachoService.getSecuencial(Number(this.tipo_documento), Number(this.id_empresa)).subscribe({
      next: (resp: any) => {
  ...
  async guardar() {
    ...
    try {
      ...
      const resp: any = await firstValueFrom(this.DespachoService.grabarTramite(formData));
  ...
  async registrarTramiteDefinitivo() {
    ...
    try {
      ...
      const resp: any = await firstValueFrom(this.DespachoService.registrarTramite(formData));
      const idTramite = resp?.id_tramite || resp?.tramite?.id_tramite || resp?.tramite?.id || resp?.id || null;
      if (!idTramite) {
      ...
      for (const p of (this.usuarios_para || [])) {
        ...
        await firstValueFrom(this.DespachoService.asginartramite(fd));
      }
  ...
  private async generarActaPdfFile(): Promise<File> {
    ...
    if (this.id_empresa) {
      try {
        empresa = await firstValueFrom(this.DespachoService.cargarempresaidVistaPrevia(Number(this.id_empresa)));
