...
export class VistaPreviaComponent implements OnInit {
  ...
  cargarEmpresa(idEmpresa: number) {
    ...
    this.DespachoService.cargarempresaidVistaPrevia(idEmpresa).subscribe({
      next: (empresa: any) => {
  ...
  generarPDF() {
    ...
    // pertenecen. Si no llegan agrupados, se usan las listas sueltas de antes.
    ...
    if (gruposAnexos.length > 0) {
      ...
      if (Array.isArray(this.data?.anexos_memorandum) && this.data.anexos_memorandum.length > 0) {
        bloquesAnexos.push({ text: `\nAnexos de este Memorándum ${this.data?.numero_memorandum || ''}`.trimEnd() + ':', style: 'label', margin: [0, 12, 0, 6] });
        bloquesAnexos.push({ ul: this.data.anexos_memorandum.map((n: any) => String(n)), fontSize: 9 });
      }
      if (Array.isArray(this.data?.anexos_tramite) && this.data.anexos_tramite.length > 0) {
        bloquesAnexos.push({ text: `\nAnexos de Trámite ${this.data?.numero_tramite_anexos || ''}`.trimEnd() + ':', style: 'label', margin: [0, 12, 0, 6] });
        bloquesAnexos.push({ ul: this.data.anexos_tramite.map((n: any) => String(n)), fontSize: 9 });
      }
    }
