...
export class NuevoTramiteComponent implements OnInit {
  ...
  private cargarTipos() {
    this.recepcionService.configTipoDocumento(this.id_empresa).subscribe({
      next: (resp: any) => {
    ...
    this.recepcionService.configTipoTramite(this.id_empresa).subscribe({
      next: (resp: any) => {
  ...
  private actualizarNumeroInterno() {
    ...
    this.recepcionService.getSecuencial(Number(this.tipo_documento), Number(this.id_empresa)).subscribe({
      next: (resp: any) => {
  ...
  async guardar() {
    ...
    try {
      ...
      const resp: any = await firstValueFrom(this.recepcionService.grabarTramite(formData));
  ...
  async registrarTramiteDefinitivo() {
    ...
    try {
      ...
      const resp: any = await firstValueFrom(this.recepcionService.registrarTramite(formData));
      const idTramite = resp?.id_tramite || resp?.tramite?.id_tramite || resp?.tramite?.id || resp?.id || null;
      if (!idTramite) {
      ...
      for (const p of (this.usuarios_para || [])) {
        ...
        await firstValueFrom(this.recepcionService.asginartramite(fd));
      }
  ...
  private async generarActaPdfFile(): Promise<File> {
    ...
    if (this.id_empresa) {
      try {
        empresa = await firstValueFrom(this.recepcionService.cargarempresaidVistaPrevia(Number(this.id_empresa)));
