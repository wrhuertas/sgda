...
export class ControlCalidadComponent {
  ...
  // (Funciones de validación de peso y estados ya no se usan en la vista eliminada)
  ...
  validarSeleccionIndividual(doc: any, event: any) {
    ...
    if (checkbox.checked) {
      // Validación de peso removida con la eliminación de restricciones en vista
  ...
  async procesarOCR(skipConfirm: boolean = false) {
  ...
  // Confirma con el mensaje solicitado y luego inicia sin segunda confirmación
  async iniciarControlCalidad() {
    const confirm = await Swal.fire({
      title: 'Confirmación',
      text: '¿Está seguro de iniciar el proceso de control de calidad?',
      icon: 'question',
      showCancelButton: true,
  ...
  // Switches (se mantienen por compatibilidad con el payload)
  ...
  // Mensajes de botón eliminados junto con la UI relacionada
