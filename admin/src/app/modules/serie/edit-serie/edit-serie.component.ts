...
ngOnInit(): void {
  if (this.SERIE_SELECTED) {
    ...
    this.plazoGestion = this.SERIE_SELECTED.plazo_gestion;
    this.plazoCentral = this.SERIE_SELECTED.plazo_central;
    this.plazoIntermedio = this.SERIE_SELECTED.plazo_intermedio;
    this.plazoHistorico = this.SERIE_SELECTED.plazo_historico;
    ...
    try {
      ...
      this.parametros = parsed.map((p: any) => ({ valor: p }));
    } catch {
    ...
    this.ficha = {
      ...
      plazoConservacion: this.SERIE_SELECTED.plazo_conservacion,
      ...
      criterios: JSON.parse(this.SERIE_SELECTED.criterios ?? '{}')
    };
    this.estado = String(this.SERIE_SELECTED.estado);


  }
  // Cargar usuario para auditoría (id_empresa, id_usuario)
...
// ====== Métodos UI: acordeón y selección ======
...
generarQR(modalTpl: any) {
  ...
  const data = {
    id_subseccion: this.idSubseccion ?? this.SERIE_SELECTED?.id_subseccion ?? null,
...
actualizarSerie() {
  ...
  const payload = {
    ...
    // Incluir selecciones locales para QR y Barra en la actualización
  ...
  this.serieservice
    .updateSerie(this.SERIE_SELECTED.id_serie, payload)
    .subscribe({
      next: (res) => {
...
// Abrir modal de Etiquetas con el componente compartido
